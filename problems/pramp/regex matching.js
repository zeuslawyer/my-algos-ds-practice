...
function isMatch(s, p) {
  s = " " + s
  p = " " + p
  let dp = Array.from(Array(s.length), () => Array(p.length).fill())
  ...
  for (let r = 1; r < s.length; r++) {
    ...
    // case 2: pat ==="*"
        ...
        // 0 occurence of the pat before *, => go 2 left
        dp[r][c] = dp[r][c - 2]

        //but update if pat before the * == char OR ".", => 1 occurence of pat before *
        if (s[r] === p[c - 1] || p[c - 1] === ".") {
