...
function helper(arr, start, end){
  ...
  while (leftInd <= rightInd){
    if (arr[leftInd] > arr[pivotInd] && arr[rightInd] < arr[pivotInd]) { // swap condition: both are not corretly positioned with ref to pivot
  ...
  let rightLength = end - (rightInd + 1)
  let leftIsShorter = leftLength < rightLength
