...
/**
 * Special :  this does not require a map to store values.
 *  runs in O(N) time but O(1) space!!!!!
s  * @param {LinkedListNode} firstNode 
 */
...
function containsCycle(firstNode) {
  ...
  while(fastRunner && fastRunner.next){
    ...
    if(fastRunner = slowRunner) return true // the fast runner has lapped the slow runner
  }

  return false;
}

// this one takes O(N) space
function $containsCycle(firstNode) {
  ...
  while(node){
    if(map[node.value] === node)  return true
